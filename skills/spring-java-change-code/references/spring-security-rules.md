...
Be careful with matcher ordering, path pattern changes, HTTP method-specific rules, default deny vs default allow behavior, role prefix conventions, authorities vs roles, method security annotations, ownership checks, tenant isolation, and admin vs user flows.
...
Do not log or expose authorization headers, cookies, refresh tokens, access tokens, session IDs, password hashes, API keys, private keys, or OAuth/OIDC claims containing sensitive user data.
