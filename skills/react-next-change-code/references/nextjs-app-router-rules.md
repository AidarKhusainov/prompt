...
Read this file when a change touches a Next.js application, especially `app/`, `pages/`, route handlers, API routes, Server Components, Client Components, server actions/functions, forms, metadata, caching, revalidation, images, fonts, middleware/proxy, runtime config, or build/deployment behavior.
...
- For Next.js 16+ projects, account for Cache Components and the `proxy.ts` rename when the repository has adopted those patterns.
...
## Server Actions / Server Functions and forms

- Use server actions/functions only when supported by the installed Next.js and React versions and consistent with the repository.
...
- For Next.js 16+ projects that already use `proxy.ts`, follow proxy naming and runtime constraints.
